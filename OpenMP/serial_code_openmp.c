...
void mttkrp(int n1, int n2, int n3,int cp, int flag,double X[n1][n2][n3], double A[][cp], double B[][cp], double C[][cp], double hat_[][cp]){
	...
	*/
	...
	if(flag==0)
		//Ahat
		#pragma omp parallel for num_threads(thread_count) default(none) shared(X, B, C, hat_, n1, n2, n3, cp) private(i,j,k,f, sum)
		for(i=0;i<n1;i++){
			for(f=0;f<cp;f++){
				for(j=0;j<n2;j++){
					for(k=0;k<n3;k++){
						// sum += X[i][j][k]*B[j][f]*C[k][f];
	...
	if(flag==1)
	//Bhat
		#pragma omp parallel for num_threads(thread_count) default(none) shared(X, A, C, hat_, n1, n2, n3, cp) private(i,j,k,f)
		for(j=0;j<n2;j++){
			for(f=0;f<cp;f++){
				hat_[j][f] = 0;
				for(i=0;i<n1;i++){
	...
	if(flag==2)
	//Chat
		#pragma omp parallel for num_threads(thread_count) default(none) shared(X, B, A, hat_, n1, n2, n3, cp) private(i,j,k,f)
		for(k=0;k<n3;k++){
			for(f=0;f<cp;f++){
				hat_[k][f] = 0;		
				for(i=0;i<n1;i++){
...
double norm(double x[], int n){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(x, sqsum) private(i) reduction(+:sqsum)
	for(i =0; i<n; i++){
		sqsum += pow(x[i],2);
	}
	return sqrt(sqsum);
...
double tensor_norm(int n1,int n2, int n3,double X[][n2][n3]){
	...
	int i,j,k;
	//parallelizable with a reduction clause
	#pragma omp parallel for num_threads(thread_count) default(none) shared(X, sqsum) private(i,j,k) reduction(+:sqsum)
	for(i =0; i<n1; i++){
		for(j = 0; j < n2; j++){
			for(k = 0; k < n3; k++){
				sqsum += pow(X[i][j][k],2);
...
void random_init(int nrowA, int ncolA, double A[][ncolA]){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(A, nrowA, ncolA) private(i,j)
	for (i=0; i<nrowA; i++){
		for (j=0; j<ncolA; j++){
...
void transpose( int nrowA, int ncolA, double A[][ncolA], double A_t[][nrowA]){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(A, nrowA, ncolA, A_t) private(i,j)
	for (i=0; i<nrowA; i++)
		for (j=0; j<ncolA; j++)
			A_t[j][i] = A[i][j];
}
...
void matmul(int nrowA, int ncolA, double A[][ncolA], int nrowB, int ncolB, double B[][ncolB], double C[][ncolB]){
	...
	// Parallelizable
	#pragma omp parallel for num_threads(thread_count) default(none) shared(C,A,B, nrowA, ncolB,nrowB, ncolA) private(i,j,k) 
	for (i=0; i<nrowA; i++){
		for (j=0; j<ncolB; j++){
			// require reduction if parallelized
			for (k=0; k<nrowB; k++){
				C[i][j] += A[i][k]*B[k][j];
			}
		}
...
void mat_vect_mul(int s1, int s2, int sv,double mat[][s2], double vec[], double result[]){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(mat, result, s1,s2) private(i,j) 
	for(i=0;i<s1;i++){
		for(j=0;j<s2;j++){
			result[i] += mat[i][j];
		}
...
void vect_mat_mul(int s1, int s2,int sv,double vec[],double mat[][s2], double result[]){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(mat, vec, result, s2, sv) private(i,j,sum)
	for(int j=0;j<s2;j++){
		sum = 0;
		for(int i=0;i<sv;i++){
			sum += mat[i][j]*vec[i];
...
double dot_product(double vec1[],int sv1,double vec2[],int sv2){
	...
	*/
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(vec1, vec2, sv1, result) private(i) reduction(+:result)
	for(i=0;i<sv1;i++){
...
void copy(int nrowA, int ncolA, double A[][ncolA], double B[][ncolA]){
	...
	//Completely parallelizable
	#pragma omp parallel for num_threads(thread_count) default(none) shared(nrowA, ncolA, A, B) private(i,j)
	for (i=0;i<nrowA; i++)
		for(j=0; j<ncolA; j++)
			B[i][j] = A[i][j];
}
...
void Hadamard(int nrow, int ncol, double A[][ncol], double B[][ncol], int symm_flag, double C[][ncol]){
	...
	if(symm_flag==0)
		#pragma omp parallel for num_threads(thread_count) default(none) shared(nrow, ncol, A, B, C) private(i,j)
...
void Cholesky_Decomposition(int nrows, int ncols, double A[][ncols], double L[][ncols])
{
	...
	#pragma omp parallel for num_threads(thread_count) default(none) private(i,j) shared(L, nrows, ncols) 
	for(i =0; i< nrows; i++)
		for( j = 0; j<ncols; j++)
			L[i][j] = 0;		
	...
	#pragma omp parallel num_threads(thread_count) default(none) private(i,j, sum) shared(L, A, nrows, ncols)
    for (i = 0; i < nrows; i++) {
    	#pragma omp for
        for (j = 0; j <= i; j++) {
            double sum = 0;
...
void inverse_lower_triangular(int nrowL, int ncolL, double L[][ncolL], double Linverse[][ncolL]){
	...
	// These two loops are probably non-parallelizable. Need to check one again
	for (int i=0; i<nrowL; i++){
		for (int j=0; j<=i;j++){
			if (i==j){
				...
				double sum = 0; int k;
				#pragma omp parallel for num_threads(thread_count) private(i,j,k) shared(L, Linverse, sum) reduction(+:sum)
				for(k =j; k<i;k++){
...
double XZ_product(int n1, int n2, int n3, int cp, double X[n1][n2][n3],double A[n1][cp],double B[n2][cp],double C[n3][cp], double lambda[]){
	...
	*/
	...
	#pragma omp parallel for default(none) private(i,k,j,f) shared(fnorm_XZ, X, A, B, C, lambda, n1, n2, n3, cp) reduction(+:fnorm_XZ)
	for(f = 0; f<cp;f++){
		for(i=0;i<n1;i++){
			for(j=0;j<n2;j++){
				for(k=0;k<n3;k++){
...
void reconstruct(int n1, int n2, int n3, int cp, double A[][cp], double B[][cp], double C[][cp],double lambda[],double Z[n1][n2][n3]){
	...
	*/
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(A, B, C, n1, n2, n3, cp, lambda, Z) private(i, j, k, f, sum) reduction(+:sum) 
	for(int i=0;i<n1;i++){
		for(int j=0;j<n2;j++){
			for(int k=0;k<n3;k++){
				sum = 0;
				for(int f=0;f<cp;f++){
					sum += lambda[f]*A[i][f]*B[j][f]*C[k][f];
				}
				Z[i][j][k] = sum;
			}
		}
...
void col_2_norm(int s1, int s2,double mat[][s2],double lambda[]){
	...
	*/
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(mat, lambda,s1,s2) private(i,j)
	for(j=0;j<s2;j++){
		lambda[j] = 0;
		for(i=0;i<s1;i++){
			lambda[j] += pow(mat[i][j],2);
		}
...
void max_col_norm(int s1, int s2, double mat[][s2], double lambda[]){
	...
	*/
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(mat, lambda, s1, s2) private(max_val,i,j)
	for(j=0;j<s2;j++){
		lambda[j] = 0;
		max_val = 1;
		for(i=0;i<s1;i++){
...
void gramschmith(int i, int L, int F, double U[][F]){
	...
	// Check this again
	#pragma omp parallel for num_threads(thread_count) default(none) shared(U, L, F, i) private(j,k, component) reduction(+:sum)
	for(j = 0; j< i; j++){
			...
			for(k =0; k<L; k++){
				sum[k] += -component*U[k][j];
			}
		}

	#pragma omp parallel for num_threads(thread_count) default(none) shared(U, sum,i) private(k)
	for (k=0;k<L;k++){
		U[k][i] += sum[k];
	}
}
...
void random_scaling_of_columns(int L, int F, double U[][F]){
	int i,k;
	double randnum;
	#pragma omp parallel num_threads(thread_count) default(none) shared(L,F,U) private(randnum, i, k)
	for(int i=0; i<F; i++){
...
void init_congruence_mat(int F,double A[][F], double congruence){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) private(i,j) shared(A, F, F, congruence)
	for (i=0; i<F;i++){
		for (j = 0; j<F; j++){
			if (i==j){
				A[i][j] = 1.0;
...
void generate_data(int n1, int n2, int n3, int cp, double congruence, double tensor[][n2][n3]){
	...
	double sum = 0.0;
	#pragma omp parallel for num_threads(thread_count) default(none) private(i,j,k,iter) shared(tensor, A, B, C) 
	for(int i = 0; i<L; i++){
		for(int j=0; j<L; j++){
			for(int k=0; k<L; k++){
				tensor[i][j][k] = 0.0;
	...
	// Add noise
	#pragma omp parallel for num_threads(thread_count) default(none) private(i,j,k) shared(eta, n_norm, tensor, sum, noise_tensor, L)
	for(int i = 0; i<L; i++){
		for(int j=0; j<L; j++){
			for(int k=0; k<L; k++){
...
int main(int argc,char* argv[]){
	...
	*/
	...
	// Generating the appropriate tensor
