...
void mttkrp(int n1, int n2, int n3,int cp, int flag,double X[n1][n2][n3], double A[][cp], double B[][cp], double C[][cp], double hat_[][cp]){
	...
	*/
	...
	if(flag==0)
		//Ahat
		#pragma omp parallel for num_threads(thread_count) default(none) shared(X, B, C, hat_, n1, n2, n3, cp) private(i,j,k,f)
		for(i=0;i<n1;i++){
			for(f=0;f<cp;f++){
				hat_[i][f] = 0;
				for(j=0;j<n2;j++){
...
double norm(double x[], int n){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(x, n) private(i) reduction(+:sqsum)
	for(i =0; i<n; i++){
		sqsum += x[i]*x[i];
	}
	return sqrt(sqsum);
...
double tensor_norm(int n1,int n2, int n3,double X[][n2][n3]){
	...
	int i,j,k;
	//parallelizable with a reduction clause
	#pragma omp parallel for num_threads(thread_count) default(none) shared(X, n1, n2, n3) private(i,j,k) reduction(+:sqsum)
	for(int i =0; i<n1; i++){
		for(int j = 0; j < n2; j++){
			for(int k = 0; k < n3; k++){
...
double dot_product(double vec1[],int sv1,double vec2[],int sv2){
	...
	*/
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(vec1, vec2, sv1) private(i) reduction(+:result)
	for(int i=0;i<sv1;i++){
...
void Cholesky_Decomposition(int nrows, int ncols, double A[][ncols], double L[][ncols])
{
	...
	double sum;
	#pragma omp parallel num_threads(thread_count) default(none) private(i,j, sum, k) shared(L, A, nrows, ncols)
    for (int i = 0; i < nrows; i++) {
        for (int j = 0; j <= i; j++) {
            double sum = 0;
...
double XZ_product(int n1, int n2, int n3, int cp, double X[n1][n2][n3],double A[n1][cp],double B[n2][cp],double C[n3][cp], double lambda[]){
	...
	*/
	...
	#pragma omp parallel for default(none) private(i,k,j,f) shared( X, A, B, C, lambda, n1, n2, n3, cp) reduction(+:fnorm_XZ)
	for(int f = 0; f<cp;f++){
		for(int i=0;i<n1;i++){
			for(int j=0;j<n2;j++){
				for(int k=0;k<n3;k++){
...
void reconstruct(int n1, int n2, int n3, int cp, double A[][cp], double B[][cp], double C[][cp],double lambda[],double Z[n1][n2][n3]){
	...
	*/
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(A, B, C, n1, n2, n3, cp, lambda, Z) private(i, j, k, f) reduction(+:sum) 
	for(int i=0;i<n1;i++){
		for(int j=0;j<n2;j++){
			for(int k=0;k<n3;k++){
				Z[i][j][k] = 0;
...
void gramschmith(int i, int L, int F, double U[][F]){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) shared(U, sum,i,L) private(k)
	for (k=0;k<L;k++){
		U[k][i] += sum[k];
	}
}
...
void initU(int L, int F, double U[][F]){
	...
	for(int i=0; i<F; i++){
		...
		// parallelizable
		int k;
		#pragma omp parallel for default(none) shared(U, norm,L, i) private(k) 
		for (int k =0; k<L; k++){
			U[k][i] = U[k][i]/norm;
		}
	}
...
void init_congruence_mat(int F,double A[][F], double congruence){
	...
	#pragma omp parallel for num_threads(thread_count) default(none) private(i,j) shared(A, F, congruence)
	for (i=0; i<F;i++){
		for (j = 0; j<F; j++){
			if (i==j){
				A[i][j] = 1.0;
...
void generate_data(int n1, int n2, int n3, int cp, double congruence, double tensor[][n2][n3]){
	...
	double sum = 0.0;
	#pragma omp parallel for num_threads(thread_count) default(none) private(i,j,k,iter) shared(tensor, A, B, C, F, L) reduction(+:sum)
	for(i = 0; i<L; i++){
		for(j=0; j<L; j++){
			for(k=0; k<L; k++){
				tensor[i][j][k] = 0.0;
